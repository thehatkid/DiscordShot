...
class Spriteset_Map
  ...
  def initialize(spriteset = nil)
    ...
    @pan_animate_timer = 0
    # Overlay text info
    @overlay_info = Sprite.new(@viewport_flash)
    @overlay_info.bitmap = Bitmap.new(640, 480)
    @overlay_info.bitmap.font.size = 20
    @overlay_info.x = 0
    @overlay_info.y = 0
    # Frame update
    update
  ...
  def update_overlay_info
    # clear bitmap
    @overlay_info.bitmap.clear

    # draw last inputs
    @overlay_info.bitmap.draw_text(-8, 10, 640, 10, "Last Inputs:", 2)
    @overlay_info.bitmap.font.size = 12
    unless $game_system.ds_last_inputs['authors'][0].nil?
      @overlay_info.bitmap.draw_text(-8, 30, 640, 10, "#{$game_system.ds_last_inputs['authors'][0]} (#{$game_system.ds_last_inputs['inputs'][0]})", 2)
    end
    unless $game_system.ds_last_inputs['authors'][1].nil?
      @overlay_info.bitmap.draw_text(-8, 45, 640, 10, "#{$game_system.ds_last_inputs['authors'][1]} (#{$game_system.ds_last_inputs['inputs'][1]})", 2)
    end
    unless $game_system.ds_last_inputs['authors'][2].nil?
      @overlay_info.bitmap.draw_text(-8, 60, 640, 10, "#{$game_system.ds_last_inputs['authors'][2]} (#{$game_system.ds_last_inputs['inputs'][2]})", 2)
    end
    unless $game_system.ds_last_inputs['authors'][3].nil?
      @overlay_info.bitmap.draw_text(-8, 75, 640, 10, "#{$game_system.ds_last_inputs['authors'][3]} (#{$game_system.ds_last_inputs['inputs'][3]})", 2)
    end
    unless $game_system.ds_last_inputs['authors'][4].nil?
      @overlay_info.bitmap.draw_text(-8, 90, 640, 10, "#{$game_system.ds_last_inputs['authors'][4]} (#{$game_system.ds_last_inputs['inputs'][4]})", 2)
    end
    @overlay_info.bitmap.font.size = 20

    # draw puzzle safe code
    if $game_variables[20] != 0 and $game_switches[49] == false
      @overlay_info.bitmap.draw_text(8, 10, 640, 10, "Safe Code: #{$game_variables[20].to_s}")
    end
  end
end
